...
public class UserGameLibraryTests
{
    ...
    {
        var entry = UserGameLibrary.Create(ValidUserId, ValidGameId, 59.99m);
    ...
    {
        var act = () => UserGameLibrary.Create(Guid.Empty, ValidGameId, 0m);
    ...
    {
        var act = () => UserGameLibrary.Create(ValidUserId, Guid.Empty, 0m);
        act.Should().Throw<UserDomainException>()
    ...
    {
        var act = () => UserGameLibrary.Create(ValidUserId, ValidGameId, -1m);
        act.Should().Throw<UserDomainException>()
    ...
    {
        var entry = UserGameLibrary.Create(ValidUserId, ValidGameId, 0m);
