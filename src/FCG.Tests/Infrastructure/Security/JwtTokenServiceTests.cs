...
public class JwtTokenServiceTests
{
    ...
    public void GenerateToken_ShouldReturnValidJwtFormat()
    {
        ...
        token.Split('.').Should().HaveCount(3, "JWT must have header.payload.signature format");
    }
    ...
    public void GenerateToken_ForUserRole_ShouldContainCorrectRoleClaim()
    {
        ...
        payload.Should().Contain(RoleType.User.ToRoleName());
        payload.Should().Contain(user.Id.ToString());
        payload.Should().Contain(user.Email.Address);
    }
    ...
    public void GenerateToken_ForAdminRole_ShouldContainAdminRoleClaim()
    {
        ...
        payload.Should().Contain(RoleType.Administrator.ToRoleName());
        payload.Should().Contain(RoleType.Administrator.ToRoleId().ToString());
    }

    [Fact]
    public void GenerateToken_ShouldEmbedRoleIdClaim()
    {
        var service = new JwtTokenService(_settings);
        var user = CreateUser(RoleType.User);
        ...
        payload.Should().Contain("roleId");
        payload.Should().Contain(RoleType.User.ToRoleId().ToString());
    }
    ...
    public void GenerateToken_ShouldSetExpirationBasedOnSettings()
    {
        ...
        expTime.Should().BeAfter(before.AddHours(1).AddSeconds(-5));
        expTime.Should().BeBefore(after.AddHours(1).AddSeconds(5));
    }

    private static User CreateUser(RoleType roleType)
        => User.Create("Test User", "test@fcg.com", "$2a$12$somehashvalue", roleType.ToRoleId());
