...
[Binding]
[Scope(Feature = "Atualização de Promoção de Jogos ADMIN")]
public class AdminCreateUserUseCaseTestsStepDefinition
{
    ...
    {
        _promo = GamePromotion.Create(Guid.NewGuid(), DiscountType.Percentage, 10, DateTime.UtcNow.AddDays(1), DateTime.UtcNow.AddDays(10));
        _promoRepoMock
    ...
    [When(@"um usuário comum tentar atualizar essa promoção")]
    public void WhenUsuarioComumTentaAtualizar()
    {
        var request = new UpdatePromotionRequest(null, null, null, null, null);
        PrepareExecution(_promo!.Id, request, _userRoleId);
    }

    [When(@"um administrador tentar atualizar uma promoção inexistente")]
    public void WhenAdminTentaAtualizarInexistente()
    {
        var request = new UpdatePromotionRequest(null, null, null, null, null);
        PrepareExecution(Guid.NewGuid(), request, _adminRoleId);
    }

    [When(@"um administrador tentar atualizar essa promoção alterando as datas")]
    public void WhenAdminTentaAlterarDatas()
    {
